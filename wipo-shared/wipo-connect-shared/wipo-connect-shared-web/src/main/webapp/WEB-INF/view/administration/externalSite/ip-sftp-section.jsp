...
<c:set var="idSfx" value="IpSftpDet" />
...
<tiles:insertDefinition name="contentTemplate">
		...
		<script type="text/javascript">
    		...
    		$(document).ready(function(){
        		...
        		$(".container${idSfx} select.form-control").selectpicker();
        	 	...
        	 	$(".container${idSfx} #formIpSftp${idSfx}").validate({
					rules:{
						"ipsFTP.host": {required:true},
						"ipsFTP.port": {required:true,digits:true},
						"ipsFTP.user": {required:true},
						"newPassword":  {required:false}
						}
             	});
         		
             	initAjaxForm(".container${idSfx} #formIpSftp${idSfx}" ,
                 		{	
	...
	<tiles:putAttribute name="body">
		<div class="container${idSfx}">
			<form:form id="formIpSftp${idSfx}" cssClass="form-inline" modelAttribute="vo" action="${ctx}/mvc/administration/externalSite/updateExternalSiteFTP.json" method="POST" enctype="multipart/form-data">
				<div class="panel panel-default ip-sftp-panel">
				<div class="panel-heading">
						<h3 class="panel-title">${msg['externalSite.ip-sftp']}</h3>
					</div>
					<div class="panel-body">
					<div class="form-row">
						<form:hidden path="ipsFTP.code" cssClass="code"/>
						<div class="form-group">
							<label for="host${idSfx}">${msg['externalSite.host']}</label>
							<form:input id="host${idSfx}" path="ipsFTP.host" cssClass="form-control hostname" />
						</div>
						<div class="form-group small">
							<label for="port${idSfx}">${msg['externalSite.port']}</label>
							<form:input id="port${idSfx}" path="ipsFTP.port" cssClass="form-control port" />
						</div>
						<div class="form-group">
							<label for="user${idSfx}">${msg['externalSite.user']}</label>
							<form:input id="user${idSfx}" path="ipsFTP.user" cssClass="form-control user" />
						</div>
						<div class="form-group">
