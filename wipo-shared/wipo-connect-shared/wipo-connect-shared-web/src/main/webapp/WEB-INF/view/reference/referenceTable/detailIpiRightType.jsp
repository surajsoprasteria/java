...
<c:set var="idSfx" value="IpiRightTypeDet" />
    		...
    		$(document).ready(function(){
				...
				$(".container${idSfx} form").validate({rules:{
					"code" : {required:true, remote: function(element) {
                		return {
                			"url": globalParams.ctxMvc + "/reference/referenceTable/checkIpiRightTypeCodeUniqueness.json",
                			"type": "post",
                			"data": {
                				"id": "${vo.id}",
                				"code": $(element).val()
             	...
             	$("#btnSave${idSfx}").click(function(e){
    	    		if (!$("#form${idSfx}").valid()){
             		...
             		$("#form${idSfx}").data("validation-type", "LOCAL"); 
           	    	$("#form${idSfx}").attr('action', "${ctx}/mvc/reference/referenceTable/saveIpiRightType.json")
           	    	$("#form${idSfx}").submit();
             	});
